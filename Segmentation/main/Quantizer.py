...
def get_quant_exp(variable, num_bits, use_abs, return_abs):
    ...
    unsigned_width = num_bits - _ACTIV_MARGIN  # for the sign bit
    ...
    def __call__(self, inputs, training, weights, **kwargs):
        ...
        if training is True:
            with tf.device('/:cpu:0'):
                ...
                quant_exp = tf.reduce_max(weights["exp_memory"]) - _ACTIV_MARGIN  # -1 for avoiding overflow
...
def apply_quantization(model, pruning_policy=None, weight_precision=None, activation_precision=None, activation_margin=None):
