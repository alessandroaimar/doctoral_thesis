...
def get_dataset(images_path, segs_path, batch_size,
                n_classes, input_height, input_width,
                output_height, output_width,
                do_augment=False,
                augmentation_name="aug_all"):
    args = (images_path, segs_path, batch_size, n_classes,
            input_height, input_width, output_height, output_width,
            do_augment, augmentation_name)

    dataset = tf.data.Dataset.from_generator(

        parser, (tf.float32, tf.float32), output_shapes=(
            tf.TensorShape((input_height, input_width, 3)),
            tf.TensorShape((output_height * output_width, n_classes))
        ),
...
def parser(images_path, segs_path, batch_size,
           n_classes, input_height, input_width,
           output_height, output_width,
           do_augment=False,
           augmentation_name="aug_all"):
    img_seg_pairs = get_pairs_from_paths(images_path.decode('ascii'), segs_path.decode('ascii'))

    random.shuffle(img_seg_pairs)
    # zipped = itertools.cycle(img_seg_pairs)

    num_samples = len(img_seg_pairs)
    num_batches = num_samples // batch_size + 1

    for (im_name, seg_name) in img_seg_pairs:

        im = cv2.imread(im_name, 1)
        seg = cv2.imread(seg_name, 1)

        if do_augment:
            im, seg[:, :, 0] = augment_seg(im, seg[:, :, 0], augmentation_name.decode('ascii'))

        try:
            X = get_image_array(im, input_width, input_height, ordering=IMAGE_ORDERING)
            Y = get_segmentation_array(seg, n_classes, output_width, output_height)

            yield np.array(X), np.array(Y)
        except Exception as e:
            print("Exception {} reading image {} mask {}, stepping over".format(e, im_name, seg_name))
...
def image_segmentation_generator(images_path, segs_path, batch_size,
    ...
    num_samples = len(img_seg_pairs)
    num_batches = num_samples // batch_size + 1
    print("Batch gen with {} batches".format(num_batches))
