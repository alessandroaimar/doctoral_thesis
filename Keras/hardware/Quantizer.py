...
def get_quant_exp(variable, num_bits, use_abs, return_abs):
    ...
    # ceil always round up including negative numbers,
...
class BFPWeightQuantizer(BFPQuantizer):
    ...
    def build(self, tensor_shape, name, layer):
        ...
        self.pruning_tensor_size_fp = tf.dtypes.cast(self.pruning_tensor_size_int, dtype=self.pruning_tensor_size_fp_dtype)
                ...
                with tf.name_scope("pruning"):
                    ...
                    def update_mask():
                        ...
                        reordered_mask = tf.map_fn(lambda x: self.update_mask_kernel(x, pruning_threshold_index), reordered_kernel,
                                                   parallel_iterations=inputs.shape[-1], back_prop=False, swap_memory=True,
                    ...
                    sel_mask = tf.cond(update_pruning, lambda: update_mask(), lambda: weights["mask"])
    ...
    def __call__(self, inputs, training, weights, **kwargs):
        ...
        if training is True:
            with tf.device('/:cpu:0'):
                ...
                weights["exp_memory"][weights["exp_memory_ptr"]].assign(new_exp)
                weights["exp_memory_ptr"].assign(new_memory_ptr)

                quant_exp = tf.reduce_max(weights["exp_memory"]) - _ACTIV_MARGIN  # - for avoiding overflow
                weights["exp"].assign(quant_exp)

                if self.use_bias:
                    weights["output_to_bias_exp"].assign(quant_exp)
        ...
        if training is False:
            with tf.device('/:cpu:0'):
                sparsity = get_sparsity_unknown_size(quantized_inputs)
                # store_op = weights["stored_tensor"].assign(quantized_inputs[0])  # TODO REMOVE, DEBUG ONLY
                weights["sparsity"].assign(sparsity)
...
class BFPBiasQuantizer(BFPQuantizer):
    ...
    def __call__(self, inputs, training, weights, **kwargs):
        ...
        if training is False:
            ...
            weights["stored_tensor"].assign(quantized_inputs)
            weights["sparsity"].assign(sparsity)
...
class BFPQuantizeConfig(QuantizeConfig):
    ...
    def set_quantize_weights(self, layer, quantize_weights):
        ...
        if layer.use_bias:
            ...
            bias_num_ops = output_num_entries
        else:
            bias_num_ops = 0

        if isinstance(layer.output, tf.keras.layers.MaxPooling2D):
            pooling_ops = output_num_entries
        else:
            pooling_ops = 0

        if layer.activation is not None:
            activ_ops = output_num_entries
        else:
            activ_ops = 0
        ...
        layer.quantized_num_ops = mac_ops + bias_num_ops + pooling_ops + activ_ops

        # print(layer.kernel.shape,layer.output_shape, mac_per_kernel,output_num_entries,layer.quantized_num_ops)
...
def apply_quantization(model, pruning_policy=None, weight_precision=None, activation_precision=None,
    ...
    with tf.name_scope("quant"):
        ...
        log.info("Weights num bits: {} - Activ num bits: {} - Activ margin: {}".format(_WEIGHTS_NUM_BITS, _ACTIV_NUM_BITS, _ACTIV_MARGIN))
        ...
        def add_quantize_annotation(layer):
            ...
            for layer_type in quantization_map:
                ...
                if isinstance(layer, layer_type):  # and not (tf.keras.activations.get("softmax") == layer.activation):
        ...
        for q_layer in quant_aware_model.layers:
            if isinstance(q_layer, QuantizeWrapper):
                quantized_num_ops = quantized_num_ops + q_layer.layer.quantized_num_ops
        ...
        log.info("Number of ops of quantized layers: {}".format(quantized_num_ops))
